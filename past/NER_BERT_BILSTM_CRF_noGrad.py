            ...
            for entry in entries:
                ...
                for line in entry.splitlines():
                    ...
                    if pieces[0] == "====":
                        words.append('[SEP]')
                        pos_tags.append('[SEP]')
                        bio_pos_tags.append('[SEP]')
                        ner_labels.append('[SEP]')
                        continue
...
class BERT_BILSTM_CRF_NER(nn.Module):
    ...
    def _get_bert_features(self, input_ids, segment_ids, input_mask):
        ...
        bert_seq_out, _ = self.rnn(bert_seq_out)
...
model = BERT_BILSTM_CRF_NER(bert_model, start_label_id, stop_label_id, len(label_list), max_seq_length, batch_size, device)

# %%
if load_checkpoint and os.path.exists(output_dir + '/ner_nograd_bert_bilstm_crf_checkpoint.pt'):
    checkpoint = torch.load(output_dir + '/ner_nograd_bert_bilstm_crf_checkpoint.pt', map_location='cpu')
    ...
    print('Loaded the pretrain NER_nograd_BERT_BILSTM_CRF model, epoch:', checkpoint['epoch'], 'valid acc:',
...
for epoch in range(start_epoch, total_train_epochs):
    ...
    if valid_f1 > valid_f1_prev:
        ...
        torch.save({'epoch': epoch, 'model_state': model.state_dict(), 'valid_acc': valid_acc,
                   ...
                   os.path.join(output_dir, 'ner_nograd_bert_bilstm_crf_checkpoint.pt'))
...
# %%
'''
Test_set prediction using the best epoch of NER_nograd_BERT_CRF model
'''
checkpoint = torch.load(output_dir + '/ner_nograd_bert_bilstm_crf_checkpoint.pt', map_location='cpu')
